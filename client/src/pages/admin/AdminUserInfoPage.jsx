...
export default function AdminUserInfoPage() {
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesInactiveFilter = !showInactiveOnly || !user.is_active;
    const matchesBlockedFilter = !showBlockedOnly || user.is_blocked;

    return matchesSearch && matchesInactiveFilter && matchesBlockedFilter;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search and Filters */}
      <AdminCard title="검색 및 필터" subtitle="사용자를 검색하고 필터링하세요">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <AdminButton
              variant={showInactiveOnly ? "warning" : "secondary"}
              onClick={() => setShowInactiveOnly(!showInactiveOnly)}
              size="sm"
            >
              {showInactiveOnly ? '비활성 계정만 보기' : '비활성 계정 보기'}
            </AdminButton>

            <AdminButton
              variant={showBlockedOnly ? "danger" : "secondary"}
              onClick={() => setShowBlockedOnly(!showBlockedOnly)}
              size="sm"
            >
              {showBlockedOnly ? '차단된 계정만 보기' : '차단된 계정 보기'}
            </AdminButton>
          </div>
        </div>
      </AdminCard>
